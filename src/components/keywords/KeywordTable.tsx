...
import { BarChart3, Loader2, ArrowUpRight, CheckSquare, Square } from 'lucide-react';
...
export default function KeywordTable({ results, locationCode }: { results: KeywordItem[], locationCode: string }) {
  ...
  const handleAudit = async (keyword: string) => {
    ...
    if (res.analysis) setAnalysisData(p => ({ ...p, [keyword]: res.analysis }));
    setLoadingKw(null);
  };
        ...
        <thead>
          <tr className="border-b-2 border-slate-900 dark:border-white">
            <th className="w-12 px-4 py-5 text-left no-print">
              <button onClick={() => setSelectedKws(selectedKws.length === results.length ? [] : results.map(r => r.keyword))}>
            ...
            <th className="w-24 px-4 py-5 text-right no-print"><span className="sr-only">Actions</span></th>
          </tr>
        </thead>
                ...
                <tr className={`group transition-all ${isSelected ? 'bg-slate-50 dark:bg-slate-900' : ''}`}>
                  ...
                  <td className="px-2 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <ArrowUpRight size={16} className="text-slate-200 group-hover:text-slate-900 transition-all cursor-pointer" />
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4 text-right no-print">
                    ...
                    >
                      {loadingKw === item.keyword ? <Loader2 size={14} className="animate-spin" /> : 'Audit'}
                    </button>
                  </td>
                </tr>
                {isExpanded && (
