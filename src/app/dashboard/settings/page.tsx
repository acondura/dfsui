  ...
  if (dfsUser && dfsPass) {
    try {
      ...
      if (res.ok) {
        ...
        status = res.status === 401 ? 'INVALID CREDENTIALS' : `ERROR ${res.status}`;
  ...
  return (
    <div className="max-w-2xl space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Configure your DataForSEO API integration.</p>
      </div>
      ...
      <div className="bg-[#0f172a] border border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        ...
        <form action={updateSettings} className="p-6 space-y-6">
          ...
          <button type="submit" className="w-full bg-blue-700 text-white py-4 rounded-xl font-bold hover:bg-blue-600 shadow-lg shadow-blue-900/20 transition-all active:scale-[0.99]">
            Update Credentials
          </button>
        </form>
      </div>
      ...
      <div className="bg-red-950/20 border border-red-900/30 rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h2 className="font-bold text-red-500 uppercase text-xs tracking-widest">Danger Zone</h2>
        </div>
        <p className="text-sm text-red-400/60 mb-6 leading-relaxed">
        ...
        <form action={deleteCredentials}>
          <button 
            type="submit" 
            className="px-6 py-2 bg-transparent border border-red-900/50 text-red-500 text-xs font-bold rounded-lg hover:bg-red-500 hover:text-white transition-all"
          >
